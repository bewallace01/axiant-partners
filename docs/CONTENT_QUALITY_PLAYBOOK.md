...
| 2026-08-01 | /sba-loans/articles/sba-504-refinance/ **(NEW)** | **Build — genuinely long-tail.** `sba 504 refinance` 140/mo **KD 0, CPC $55.29** (highest CPC in the whole dataset) + `sba 504 refinance program` 90/KD 1, `sba 504 loan refinance program` 90/KD 5, `sba 504 refinance rules` 40/KD 0, `sba 504 refinance guidelines` 40/KD 6. Cannibalization check: **12 existing refinance pages**, but **zero** target 504 refi — real gap | exact-match title/H1; the four rule/guideline variants carried in H2s + FAQ | 1,498 words. Substance: two-year debt test, 12-month current-payment window, **85% eligible-original-use test** (the one that surprises people — blended facilities fail it), 51% occupancy, ~90% LTV cap, cash-out restricted to *eligible operating expenses* not owner distributions, other-secured-debt rollup, 60–90 day close, decline reasons | clean | routes the ineligible-cash-out reader to conventional cash-out / RE-secured pages rather than pretending 504 fits |
| 2026-08-01 | /commercial-real-estate-loans/articles/typical-commercial-real-estate-loan-rates-2026/ | **Optimize (vocabulary)** — page said "commercial real estate loan rates" throughout with **one** mention of "commercial mortgage rate". Targets: `commercial mortgage interest rates` 2,900/KD 20, `current commercial mortgage rates` 1,000/KD 21, `30 year commercial mortgage rates` 880/KD 14, `loan rates for commercial property` 4,400/KD 14 | title/og/tw lead with "Commercial Mortgage Rates"; H1 keeps the CRE phrasing it already ranks on | new H2 on term-length pricing (5yr cheapest, 10yr +25–75bps, 30yr rare outside agency/504 — and usually the *amortization* not the rate lock) + 4 FAQs mirrored in schema (5→9) | clean | same play as the calculator: lead the title with the unowned phrasing, keep the H1 exact-match |
| 2026-08-01 | /real-estate-secured-business-loan.html | **Optimize** — `commercial real estate equity loan` / `equity loan for commercial property` / `equity loan on commercial property` / `commercial property equity loan` all **170/mo, KD 0–1, CPC $39.71**; page already had a commercial-equity section but not the exact phrasings | H2 retargeted to exact-match "Commercial Property Equity Loan"; four variants named in body; 2 FAQs added (11→13) | already strong | clean | root service page — note the article-template QA checks for "Related Resources" and `../` sibling links **do not apply**; no root page on the site has them |
| 2026-08-01 | /sba-loans/articles/sba-7a-vs-504-loan/ **(MERGE)** | **Consolidate — the pair actually holding the 504 cluster down.** Third duplicate found: `sba-504-vs-7a-decision-tree` (1,442 words, **2 impressions @ pos 3.5**, 33 inbound links) vs this page (1,452 words, **498 impressions @ pos 57.0**, 47 inbound links). Near-identical length and title, same query | DataForSEO: both orderings (`sba 504 vs 7a` / `sba 7a vs 504`) are **590/mo at KD 0–2** — same Ads group, so ordering is free. Title retargeted to lead 504-first; H1 keeps the 7(a)-first exact match, so the page carries both orderings | folded in the retiring page's three unique sections: the **decision tree** (the real differentiator), **combined 504+7(a) structuring** (10–15% total equity across both programs), and **when conventional beats both** (sub-30-day closes, >$5M, no-PG). 1,452 → 1,687 words, 9.7 numbers per 100 words | clean | 52 inbound links repointed across **four** encodings; hub ItemList 30 → 29 |

| 2026-08-02 | /dscr-loans.html **(DECISION: fold, do not build)** | **Build-or-fold call** on `dscr loan mean` / `means` / `meaning` 8,100/mo KD 27 and `what is a dscr loan` 6,600/KD 13 | The hub already carries an H2 titled **"Quick Answer: What Is a DSCR Loan?"** with a front-loaded citable definition. A separate definitional page would have cannibalized the hub's own strongest section for that exact query | folded the `means`/`meaning` phrasings into the hub FAQ instead (7 -> 9 entries, schema rebuilt from visible content) | clean | **third Build-to-Optimize conversion in this repo.** The pattern holds: check whether an existing page already answers the query well before adding a URL. Fewer, stronger pages |

> **The diagnostic that matters:** one URL at 498 impressions/position 57 while its twin sits at position 3.5 on 2 impressions is the signature of a split — Google picking the weaker page, neither accumulating authority. When a cluster ranks 50+ on real impressions, look for the duplicate *before* rewriting content. All three merges in this branch showed this pattern; the content was never the problem.

> **Breadcrumbs:** schema-only `BreadcrumbList` was inherited from the article template; **visible** `nav.crumbs` existed on only 34 `/articles/` pages using a *different* template (`--ink-soft` vars). Added visible breadcrumbs to all 8 pages in this batch, ported to the article template's own variables, and synced every `BreadcrumbList` leaf name to the visible leaf text — four were stale after retargeting.

> **Long-tail honesty:** not every low-KD win is long-tail. `sba 504 lenders` (8,100/mo) and `commercial mortgage broker` (1,600/mo) are head/mid-tail terms that happened to be KD 19 and KD 0. Only the 504-refinance cluster here is genuinely long-tail. Label them accurately in planning so the mix stays deliberate.
