...
def convert(path: Path, dry=False):
    ...
    # A noindex is a decision somebody made, not boilerplate to re-template over.
    # The v2 head hardcoded "index, follow, ...", which quietly published 27
    # deliberately hidden pages (the whole business-growth cluster) the first
    # time this ran. Keep the source's directive whenever it says noindex, and
    # otherwise use the v2 default, which adds the snippet/image-preview hints.
    src_robots = s.find("meta", attrs={"name": "robots"})
    src_robots_val = (src_robots.get("content") or "").strip() if src_robots else ""
    robots_val = (src_robots_val if "noindex" in src_robots_val.lower()
